...
from cyberbattle._env.shared_cyberbattle_env import EnvironmentBounds
from typing import Optional, List, Tuple
import enum
import numpy as np
from gym import spaces, Wrapper
from numpy import ndarray
import cyberbattle._env.shared_cyberbattle_env as cyberbattle_env
import logging
...
class Feature_active_node_properties(Feature):
    ...
    def get(self, a: StateAugmentation, node) -> np.ndarray:
        ...
        # Check if 'discovered_nodes_properties' key exists in the observation
        if 'discovered_nodes_properties' in observation:
            node_prop = observation['discovered_nodes_properties']

            assert node < len(node_prop), f'invalid node index {node} (not discovered yet)'

            # Remap to get rid of the unknown value (2): 1->1, 0->0, 2->0
            remapped = np.array(node_prop[node] % 2, dtype=np.int_)
            return remapped
        else:
            # Return a default value if 'discovered_nodes_properties' is not present
            return np.zeros(self.env_properties.property_count, dtype=np.int_)  # Default value, adjust as needed
...
class Feature_active_node_age(Feature):
    """Feature for the age of the active node"""

    def __init__(self, env_properties):
        super().__init__(env_properties, [env_properties.maximum_node_count])
    ...
    def get(self, a: StateAugmentation, node) -> np.ndarray:
        ...
        # Check if 'discovered_node_count' key exists in the observation
        if 'discovered_node_count' in observation:
            ...
            # Return a default value if 'discovered_node_count' is not present
            return np.array([0])  # Default value, adjust as needed
...
class Feature_discovered_nodeproperties_sliding(Feature):
    ...
    def get(self, a: StateAugmentation, node) -> np.ndarray:
        ...
        if 'discovered_node_count' in observation:
            ...
            # print(node_prop)
            # Process the last window of entries
            node_prop = np.array(observation['discovered_nodes_properties'])[:n]
            # Check if node_prop is 1D or 2D and handle accordingly
            if node_prop.ndim == 1:
                # Handle the 1D case
                node_prop_window = node_prop[-self.window_size:]
            elif node_prop.ndim == 2:
                # Handle the 2D case
                node_prop_window = node_prop[-self.window_size:, :]
            else:
                raise ValueError("Unexpected number of dimensions in node_prop")
            ...
            # Return a default value if 'discovered_node_count' is not present
            return np.zeros(self.env_properties.property_count, dtype=np.int32)  # Default value, adjust as needed
...
class Feature_discovered_ports_sliding(Feature):
    """Compute the sliding mean of discovered ports"""
    window_size = 3
    ...
    def get(self, a: StateAugmentation, node=None):
        ...
        # Check if 'discovered_ports' key exists in the observation
        if 'discovered_ports' in observation:
            discovered_ports = observation['discovered_ports']
            # Compute the sliding mean of the last few entries
            sliding_mean = np.mean(discovered_ports[-self.window_size:])
            return np.array([sliding_mean])
        else:
            # Return a default value if 'discovered_ports' is not present
            return np.array([0])  # Default value, adjust as needed
...
class Feature_discovered_notowned_node_count(Feature):
    ...
    def get(self, a: StateAugmentation, node=None):
        ...
        if 'discovered_node_count' in observation:
            ...
            node_props = np.array(observation['discovered_nodes_properties'][:discovered])
            if node_props.ndim == 1:
                node_props = node_props.reshape(1, -1)
            if node_props.ndim == 2:
                owned = np.count_nonzero(np.all(node_props != 2, axis=1))
            else:
                raise ValueError("node_props is not a 2-dimensional array as expected")
...
class FeatureEncoder(Feature):
    ...
    def feature_vector_of_observation_at(self, a: StateAugmentation, node: Optional[int]) -> np.ndarray:
        """Return the current feature vector"""
        feature_vectors = []
        for i, feature in enumerate(self.feature_selection):
            fv = feature.get(a, node)
            print(f"Feature {i} ({feature.name()}): shape={fv.shape}, vector={fv}")
            if fv.ndim == 1:  # Ensure 2D shape for concatenation
                fv = fv.reshape(1, -1)
...
def owned_nodes(observation):
    """Return the list of owned nodes. If 'nodes_privilegelevel' key is not present, return an empty array."""
    if 'nodes_privilegelevel' in observation:
        return np.nonzero(observation['nodes_privilegelevel'])[0]
    else:
        return np.array([])
...
class AgentWrapper(Wrapper):
    ...
    def __init__(self, env: cyberbattle_env.CyberBattleEnvTest, state: StateAugmentation):
        super().__init__(env)
        self.state = state
