...
from cyberbattle.env_wrappers.environment_event_source import IEnvironmentObserver, EnvironmentEventSource
from cyberbattle.env_wrappers.reward_store import IRewardStore
...
class AttackerEnvWrapper(gym.Env, IRewardStore, IEnvironmentObserver):
    ...
    def __init__(self,
        ...
        self.get_privilege_level_array = cyber_env.get_privilege_level_array
        ...
        self.node_count = cyber_env.node_count
    ...
    def __create_observation_space(self, cyber_env: CyberBattleEnv) -> gym.Space:
        ...
        # Ensure the key exists before attempting to delete it
        if 'credential_cache' in observation_space:
            del observation_space['credential_cache']

        # Similarly, check for other keys before deleting
        if 'discovered_nodes' in observation_space:
            del observation_space['discovered_nodes']

        if 'explored_network' in observation_space:
