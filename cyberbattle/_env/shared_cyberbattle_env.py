...
#this is cyberbattle_env file refactored
...
def sourcenode_of_action(x: Action) -> int:
    ...
    if 'local_vulnerability' in x:
        ...
        # Handle the case where none of the expected attributes are present
        raise ValueError("Action does not contain any of the expected attributes: 'local_vulnerability', 'remote_vulnerability', 'connect'")
...
class CyberBattleEnvTest(gym.Env):
    metadata = {'render.modes': ['human']}

    @property
    def node_count(self) -> int:
        """Returns the total number of nodes in the environment's network."""
        return len(self.__environment.network.nodes)
    @property
    ...
    def __init__(self,
        ...
        self.observation_space = spaces.Dict({
            ...
            'nodes_privilegelevel': spaces.MultiDiscrete([CyberBattleEnvTest.privilege_levels] * maximum_node_count),
    ...
    def _combine_observations(self, attacker_observation, defender_observation):
        """
        Combine observations from attacker and defender.
        """
        combined_observation = {
            'attacker': attacker_observation,
            'defender': defender_observation
        }
    ...
    def __execute_action(self, action: Action) -> actions.ActionResult:
        ...
        print("attacker Action being executed:", action)
        if action == 'Reimage Node':
            print('Defender action called')
        assert 1 == len(action.keys()), "The action must contain exactly one action type"
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, StepInfo]:
        ...
        attacker_observation = self.__get_blank_observation()
        defender_observation = {}
        reward=0
        try:
            if 'action_type' in action:
                #defender's action
                print("Executing defender action", action)
                defender_observation, defender_reward, defender_done, defender_info = self.__defender_agent.step(action)
                #update environment state based on defender's action
                self.__update_environment_state_from_defender(action, defender_info)
                #print(defender_observation)
                reward=defender_reward #new
                print('defender reward',defender_reward)
                print('defender info',defender_info)
            elif any(key in action for key in ['connect','remote_vulnerability', 'local_vulnerability']):
                print("Executing attacker action",action)
                result = self.__execute_action(action)
                print('attacker action: ', result)
                attacker_observation, attacker_reward = self.__observation_reward_from_action_result(result)
                reward=attacker_reward #new
                #print("Attacker observation:", attacker_observation)
                print("Attacker reward:", attacker_reward)
                #no defender action provided, use default values
                defender_reward = 0
                defender_done = False
                defender_info = {}

            self.__done = self.__check_termination_conditions(defender_done)

            info = self.__compile_step_info(duration, defender_info)
            ...
            reward = 0
            self.__done = True  # Terminate the episode on critical errors

        #combine observations from both attacker and defender
        combined_observation = self._combine_observations(attacker_observation, defender_observation)
        #print("Combined observation:", combined_observation)

        return combined_observation, reward, self.__done, info

    def __update_environment_state_from_defender(self, defender_action, defender_info):
        """
        Update the environment state based on the defender's actions.
        """
        if defender_action['action_type'] == 'reimage_node':
            node_id = defender_action['node_id']
            #reimage the node in the environment
            self.environment.reimage_node(node_id)
        elif defender_action['action_type'] == 'update_firewall':
            node_id = defender_action['node_id']
            new_firewall_settings = defender_info.get('firewall_settings', {})
            #update firewall settings in the environment
            self.environment.update_firewall(node_id, new_firewall_settings)

    def __check_termination_conditions(self, defender_done):
        """
        Check if the simulation should end based on attacker's or defender's goals.
        """
        return (self.__attacker_goal_reached() or
                self.__defender_constraints_broken() or
                defender_done or
                self.__defender_goal_reached())

    def __calculate_combined_reward(self, attacker_reward, defender_reward):
        """
        Calculate the final reward for the step.
        """
        combined_reward = attacker_reward + defender_reward
        if self.__done:
            #additional rewards or penalties upon termination
            if self.__attacker_goal_reached():
                combined_reward += self.__WINNING_REWARD
            else:
                combined_reward += self.__LOSING_REWARD
        return max(0., combined_reward)

    def __compile_step_info(self, duration, defender_info):
        """
        Compile information about the current step.
        """
        return StepInfo(
            description='CyberBattle simulation',
            duration_in_ms=duration * 1000,  # Convert to milliseconds
            step_count=self.__stepcount,
            network_availability=self._defender_actuator.network_availability,
            defender_info=defender_info
        )

    def is_node_infected(self, node_id):
        #fetch the node information directly from the environment's network nodes
        node_data = self.network.nodes[node_id]['data']
        #return the value of agent_installed attribute directly
        return getattr(node_data, 'agent_installed', False)

    def get_firewall_status(self, node_id):
        """Get the firewall status of the specified node as an integer (1 for on, 0 for off)."""
        node_data = self.network.nodes[node_id]['data']
        firewall_status = getattr(node_data, 'firewall', None)
        if firewall_status is not None:
            return 1
        else:
            #default to 0 (off) if firewall status is not available
            return 0

    def get_service_status(self, node_id):
        """Get the service status of the specified node."""
        node_data = self.network.nodes[node_id]['data']
        return getattr(node_data, 'service', 0)


    @property
    def network_availability(self):
        """Public method to access network availability."""
        return self._defender_actuator.network_availability
